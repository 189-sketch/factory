...
func TestBinaryHandlesClosedOutputPipe(t *testing.T) {
	...
	command := exec.Command(binary, "worker", "run", "--agent=plan", "--repo="+repository, "--config="+workerPath)
...
func setupBinaryRun(t *testing.T, directory string, agentCommand []string, timeout string) (string, string, string) {
	...
	if err := os.WriteFile(filepath.Join(directory, "plan.md"), []byte("run the configured agent\n"), 0o600); err != nil {
		t.Fatal(err)
	}
