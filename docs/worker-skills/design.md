...
Factory will not implement its own skill discovery, schema, prompt compilation, storage,
...
This keeps Factory focused on admission, execution, supervision, and durable run state.
...
That is acceptable for the current personal, local factory. If reproducible multi-runner
provisioning becomes necessary, it should be solved in runner configuration management,
not by adding a second skill runtime to Factory.
...
A Factory-managed skill system would duplicate those native implementations. Factory
...
This decision covers where skills belong and what Factory must not own. It does not
...
```mermaid
flowchart LR
    DEF[Factory agent definition] --> WORKER[Worker]
...
Factory resolves and renders the durable agent prompt. The chosen coding harness resolves
its own skills after the worker starts it. Factory does not read skill files or add skill
...
through a new Factory abstraction.
...
When different Factory agents genuinely need isolated skill sets, configure separate
logical executors or native harness profiles if the harness supports them. Factory may
select those executors by name, but it does not create or manage the profiles.

### Prompt responsibilities

The durable Factory prompt states the agent's role, safety rules, workflow, and required
...
- `INV-1`: Factory never reads, parses, copies, compiles, stores, or serves native skill
  files.
- `INV-2`: Adding or changing a native skill does not change Factory's agent hash unless
  the configured Factory prompt, executor, or timeout also changes.
...
- `INV-5`: Factory remains compatible with executors that do not support skills.

### Requirements

- Do not add skill fields to `config.toml`, `worker.toml`, the worker protocol, SQLite, or
  the control-plane API.
- Do not scan `~/.factory`, a worker home directory, or the target repository for skills.
- Do not copy native skill files during `factory init`.
...
Factory does not preflight native skill availability. If a prompt explicitly requires a
missing skill, the coding agent reports the problem or fails according to its native
behavior. Factory records the resulting output and process state without interpreting it.

Skill changes take effect according to the harness lifecycle. A newly started executor
usually sees the current runner and repository files. Factory does not snapshot skills
...
executor process does not have. Factory must continue to rely on worker credentials,
repository scope, operating-system permissions, and repository protections for real
enforcement.

The operational cost is configuration drift. For the current single-user local factory,
...
bootstrap process, or an immutable runner image. Factory should only gain visibility into
skill versions if a future operational problem proves that external provisioning is not
enough.

## 8. Acceptance criteria

- `AC-1`: No Factory code, configuration schema, protocol, database, or API change is
  required for skills.
...
- `AC-4`: A runner without a particular skill can still execute agents that do not
  require it.
- `AC-5`: Factory records missing-skill output and terminal process state without adding
...
No Factory implementation test is needed because this decision adds no product behavior.
...
3. run the same request through `factory run`;
4. when using the control plane, submit the same request and confirm the managed worker
   behaves the same way.

These checks verify runner provisioning. They should not become shared Factory protocol
tests.

## 10. Tradeoffs

- Factory stays smaller and inherits new native skill features without product changes.
...
- Factory cannot show a complete skill inventory or prove which native skill version a
  run used.
- Agent portability depends on using skills supported by the selected executor.
- Project-native instructions may affect the executor independently of the durable Factory
  prompt. This is already true because the executor runs in the target repository.

The simplicity is worth these costs for the current local factory.
...
- Where should runner setup be versioned? Recommended default: the existing personal
  dotfiles or machine bootstrap repository, not this Factory runtime repository.

Neither question requires a Factory feature.

## 12. Out of scope

- A Factory skill schema or registry.
...
- [Warp factory definition syntax](https://docs.warp.dev/factories/factory-as-code/)
- [Warp issue-to-PR example](https://github.com/warpdotdev/warp-factory-examples/tree/main/examples/02-sdlc-issue-to-pr)
