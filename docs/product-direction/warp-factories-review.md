...
This review compares Machinist with the public Warp Factories product and examples. It is
a direction-setting document, not a claim that Machinist should copy Warp feature for
feature. Machinist is a local, personal system. Warp is a hosted team platform in early
access.
...
Machinist already has a sound execution core and the same control-plane versus execution-
plane split. Its coding harnesses already support composable skills, so Machinist should let
...
| Area | Warp Factories | Machinist today | Direction |
| --- | --- | --- | --- |
...
| Definitions as code | A versioned tree owns factory, agent, automation, runner, scorer, and skill definitions. | TOML and Markdown files own agents and pipelines. Worker files own executors and repositories. Native harness configuration owns skills. | Keep each file set as the source of truth for the layer it already owns. Do not pull harness features into Machinist without a proven need. |
| Skills | Shared skills apply to all agents. Agent-scoped skills apply to one role. Each skill is a `SKILL.md` with metadata and optional support files. | Codex, Claude, and other configured coding agents already provide native skill discovery and invocation. | Install and version native skills on the runner or in the target repository. Do not add a Machinist skill schema, compiler, registry, or protocol. |
...
| Execution | Hosted runners or self-hosted workers provide isolated compute. | Workers execute against pre-existing local worktrees with their local credentials. | Local-first is a feature. Add sandbox profiles only when Machinist can enforce them across executors. |
...
| Observability | Work-item stages, runs, PR links, autonomy, cycle time, cost, and scorer results are visible. | Jobs, runs, workers, duration, and optional token usage are visible. Results and event logs are stored but have no control-plane read surface. | Expose stored output, then add outcome and PR identity before adding more aggregate charts. Duration and tokens without outcome quality are incomplete. |
...
Machinist should not parse, copy, compile, store, or synchronize skills. This avoids
...
Machinist's job is currently an execution record, which is simpler and correct for V1. If
Machinist later accepts events or follow-up messages, it should add a work item above jobs
...
stable Machinist ID. Repeated deliveries should continue the same item or be rejected as
...
Only after these are reliable should Machinist derive autonomy or cost-per-PR views.

### 4. Reviewed self-improvement

Warp uses scorer failures to propose changes for review rather than silently rewriting
its factory. Machinist should keep the same human gate. Evals may identify a weak prompt or
...
semantics. Machinist should add integrations only after work-item identity exists. A small
GitHub issue intake path is more useful than a generic automation language with weak
delivery guarantees.

## What Machinist should not copy yet

- Hosted multi-tenant execution. The current goal is a trusted local Machinist.
...
- A Machinist-specific skill schema, compiler, registry, or synchronization layer. Native
...
Machinist is structurally ready to replace a personal, local older factory after the known
...
1. Rebuild and reinstall Machinist with Go 1.26.6 or newer. Go binaries include the standard
...
2. Merge the checked-in CI workflow, confirm its Linux, macOS, frontend, race, vet, and
   build checks pass on GitHub, and require its final `check` job on the default branch.
3. Stop and drain the older installation, back up `~/.factory`, and follow the
   [migration guide](../migration-from-factory.md). Machinist deliberately initializes a
   separate `~/.machinist` directory and does not import incompatible legacy state.

These are operational gates, not reasons to redesign the execution core.

## Recommended order

1. Clear the three readiness gates and use the current Machinist for real work.
